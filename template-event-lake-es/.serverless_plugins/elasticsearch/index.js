...
class Plugin {
  constructor(serverless, options) {
    this.hooks = {
      // 'before:deploy:deploy': createServiceLinkedRole.bind(null, serverless, options),
...
const createServiceLinkedRole = async (serverless, options) => {
  ...
  const response = await client.getRole({
    ...
    .promise()
    .catch(() => {
      return client.createServiceLinkedRole({
        AWSServiceName: 'es.amazonaws.com'
      }).promise();
    })

  serverless.cli.log(`Service Linked Role created (${response.Role.RoleName})`);
}
